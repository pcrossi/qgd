#!/usr/bin/env python3
"""Q43 — derivação reduzida de H1 por mistura geométrica de harmônicos.
...
def main() -> None:
    ...
    for symbol, (role, ratio) in lepton_map.items():
        ...
        a = evaluate(H, c, m)
        out = BASE / f"background_leptonico_h1mix_{symbol}_q43.npz"
        np.savez(
            out,
            H=H,
            c=c,
    ...
    lines = [
        r"# Q43 — derivação reduzida de \(H_1\) por mistura harmônica",
        "",
        "## Classificação",
        "",
    ...
    report = BASE / "saida_h1_mistura_q43.md"
