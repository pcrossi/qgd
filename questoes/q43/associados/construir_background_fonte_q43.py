#!/usr/bin/env python3
"""Q43 — background leptônico reduzido e mapa magnético físico.
...
1. usa a truncagem Galerkin oficial já implementada em
   `hessiana_oficial_galerkin_q43.py`;
...
6. salva NPZs compatíveis com `extrair_canal_superior_q43.py`.
...
GALERKIN_PATH = BASE / "hessiana_oficial_galerkin_q43.py"
...
def load_galerkin_module():
    spec = importlib.util.spec_from_file_location("q43_galerkin", GALERKIN_PATH)
...
def main() -> None:
    ...
    np.savez(BASE / "background_galerkin_busca_q43.npz", H=H_full, c=c_full, m_perp=m_perp_full, gamma0=np.array([1.0]), x_star=x_best)
    ...
    for symbol, (role, ratio) in lepton_map.items():
        ...
        out = BASE / f"background_leptonico_estavel_{symbol}_q43.npz"
        np.savez(
            out,
            H=H,
            c=c,
    ...
    lines: list[str] = [
        "# Q43 — background leptônico reduzido e mapa magnético",
        "",
        "## Classificação",
        "",
    ...
    report = BASE / "saida_background_fonte_q43.md"
