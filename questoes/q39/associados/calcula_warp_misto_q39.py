#!/usr/bin/env python3
"""Q39 — critério Schur para o caso warped/misto.

Calcula:

    m_perp^2 = C_gamma*tau/R_max^2 - sum(c_i a_i^2)
    j_mix = sum(b_i a_i)
    schur = j_mix^2 / m_perp^2
    status = subcritico se schur < lambda_B_gap
...
def evaluate(inp: MixedInput) -> dict[str, float | str]:
    ...
    schur = (j_mix * j_mix) / m_perp2
    ratio = schur / inp.lambda_b_gap
    if schur < inp.lambda_b_gap:
        status = "subcritico"
    elif math.isclose(schur, inp.lambda_b_gap, rel_tol=1e-12, abs_tol=1e-12):
        status = "critico"
    else:
        status = "supercritico"
    return {
...
def main() -> None:
    ...
    report = base / "saida_warp_misto_q39.md"
