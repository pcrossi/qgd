...
Sim, no setor físico reduzido de Madelung. A ação oficial produz diretamente
o operador geométrico de amplitude que, depois do pullback físico e da
normalização cinética do laboratório, torna-se o potencial de Bohm. A forma
temporal completa de Hamilton--Jacobi--Bohm usa a mesma polarização
canônica já isolada na Questão 10.
...
\[
\boxed{
\text{Questão 11 fechada condicionalmente no setor Madelung reduzido.}
}
\]

A ação oficial da GDQ não é alterada. O que foi demonstrado diretamente nela
é que a variação da densidade, escrita por \(q=\ln\rho\), produz o operador
geométrico:
...
Na redução física de Madelung, com pullback temporal, normalização cinética,
bordos compatíveis e polarização canônica do laboratório, esse operador entra
com o coeficiente usual e a variação reduzida gera:
...
Portanto, o status conservador é:

1. derivação do operador de Bohm a partir da densidade: demonstrada;
2. Hamilton--Jacobi--Bohm temporal completa: demonstrada no setor Madelung
   reduzido;
3. validade off-shell universal para toda solução da ação oficial: não
   afirmada.
