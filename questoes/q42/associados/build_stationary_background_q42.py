#!/usr/bin/env python3
"""Constrói e verifica o shrinker gaussiano da Q42 na fatia C^2."""
...
def build(tau: float, r_c: float, r_max: float, points: int) -> dict[str, np.ndarray]:
    if tau <= 0 or r_c < 0 or r_max <= r_c or points < 20:
        raise ValueError("parâmetros geométricos inválidos")
    ...
    # Componentes radial e esférica da equação de shrinker.
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).with_name("background_bulk_q42.npz"),
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path(__file__).with_name("saida_background_bulk_q42.md"),
    )
    args = parser.parse_args()
    data = build(args.tau, args.r_c, args.r_max, args.points)
    np.savez(args.output, **data)
    normalization = float(np.trapezoid(data["measure_radial"], data["r"]))
    # r_max finito: comparar com a unidade, registrando o corte de cauda.
    max_residual = max(
    ...
    text = "\n".join(
        [
            "# Background estacionário de bulk — Q42",
            "",
            f"- tau: `{args.tau:.12e}`",
            f"- r_c: `{args.r_c:.12e}`",
            f"- r_max: `{args.r_max:.12e}`",
            f"- F0: `{float(data['f0']):.12e}`",
            f"- maior resíduo das equações: `{max_residual:.3e}`",
            f"- normalização no domínio truncado: `{normalization:.12e}`",
            f"- fluxo normal no estômato: `{float(data['boundary_flux']):.12e}`",
            f"- curvatura média K: `{float(data['mean_curvature']):.12e}`",
            f"- curvatura ponderada K-n(F): `{float(data['weighted_mean_curvature']):.12e}`",
            "",
            "O resíduo nulo verifica o bulk. A condição de bordo livre é",
            "K-n(F)=0; ela ocorre somente quando r_c=sqrt(6 tau).",
            "",
        ]
    )
