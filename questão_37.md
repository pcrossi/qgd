...
a constante de estrutura fina $\alpha$ emerge como o acoplamento efetivo de
uma **direção $U(1)$ selecionada no toro interno**. Essa direção, sua
normalização cinética e a carga mínima são determinadas pelo **background
espectral de Ricci–Bismut**.

O valor numérico só pode ser obtido **depois** de resolver o problema
espectral
...
para $(g_*,B_*,f_*,R_{a,*})$. Até lá, a GDQ fornece a **estrutura da
derivação**, mas não o número $1/137$ como teorema fechado.

A fórmula fechada do Capítulo 29,

\[
\alpha=\frac{9}{8\pi^4}\left(\frac{\pi^5}{1920}\right)^{1/4},
\]

**não pode ser usada ainda como prova fechada**, porque foi construída sobre
a compactificação cosmológica \(T^5\times S^3\), enquanto a geometria local
oficial da GDQ é \(\mathbb R^4\times T^4\). Essa rota pode ser revisitada
depois para o cálculo global de \(\alpha\), mas precisa de uma ponte
matemática explícita entre a compactificação de Einstein e a normalização
local do setor \(U(1)\).
...
A Questão 37 está **parcialmente resolvida no nível estrutural** e
**aberta no nível numérico**.
...
A geometria usada no Capítulo 29 e em `respostas3.md`/`respostas4.md` é
...
A decomposição proposta em `respostas4.md`,
...
## 9. Implicações para `respostas3.md` e `respostas4.md`

Os documentos `respostas3.md` e `respostas4.md` concluem que a Questão 9
...
Além disso, `respostas4.md` propõe uma "integral explícita" que resulta
em $120$, não $137$, evidenciando o risco de *over-engineering* na
defesa do inteiro isolado.

**Conclusão atualizada:** a resolução apresentada em `respostas3.md` e
`respostas4.md` é uma **rota cosmológica auxiliar** para \(\alpha\). Ela não
...
`teorema_heranca_normalizacao_eletromagnetica.md`; o que permanece aberto é
...
Para obter o valor numérico de $\alpha$ na GDQ oficial, é necessário:
...
5. comparar a fórmula cosmológica proposta com $Z_Q^E$ sem inserir o valor
   experimental;
6. calcular separadamente respostas efetivas dependentes da resolução, caso
   sejam necessárias para comparação experimental.

Até que esses passos sejam completados, a Questão 37 permanece
**estruturalmente resolvida e numericamente aberta**.

---

## 11. Teorema condicional de herança da normalização

O documento `teorema_heranca_normalizacao_eletromagnetica.md` corrige uma
...
`37p/derivacao_ZQ_global_acao_oficial.md` efetuou a redução quadrática da ação
...
Hessiana permanece positiva. Isso seleciona o DtN warped--Bismut completo como
o cálculo faltante; a aproximação de 4-bola redonda ainda não é a prova final.
Ver `37p/rota_schur_dtn_global.md`.

---

## 13. Canal fotônico massless

`37p/teorema_canal_fotonico_massless.md` formulou o transporte correto sem
...
`37p/identificacao_formula_cosmologica_hessiana.md` auditou diretamente os
...
O documento `37p/interpretacao_media_einstein_formula_legada.md` preserva o
...
estatuto na GDQ é condicional: ainda se deve derivar da Hessiana global tanto
a uniformidade dos pesos das câmaras quanto o projetor $9/(8\pi^4)$.
...
quociente é $1/1920$ da integral no recobrimento. Resta nessa rota somente
derivar o projetor isotrópico $9/(8\pi^4)$ da Hessiana global.

O projetor foi obtido no setor axial coerente. O quarto momento de Haar em
$S^3$ fornece
