#!/usr/bin/env python3
"""
GDQ — Chapter 15 / 8D hierarchy response by Schur.

Classification:
    symbolic-numerical evaluation of the effective 8D formula.
...
The script evaluates:
    ...
    R_l^(8) = R_l^(0) - sigma_l,
    |sigma_l| <= Delta_Schur,
    dR_tau/dR_mu under fixed Q.

Does not use experimental masses to construct the numbers.
"""
...
def main() -> None:
    ...
    scenarios = [
        ("product", 0.0, 1.0),
        ("subcritical_weak", 0.1, 0.99),
        ("subcritical_4channels", 0.4, 0.96),
    ]
    ...
    lines = [
        ...
        f"- $R_\\mu^{{(0)}} = {rmu0:.15f}$",
        f"- $R_\\tau^{{(0)}} = {rtau0:.15f}$",
        f"- $Q(R_\\mu^{{(0)}},R_\\tau^{{(0)}}) = {q_value(rmu0, rtau0):.15f}$",
        "",
        "## Linear Response of Saturation",
        "",
        "$$",
        f"\\partial Q/\\partial R_\\mu = {dq_mu:.15e},",
        "$$",
        "$$",
        f"\\partial Q/\\partial R_\\tau = {dq_tau:.15e},",
        "$$",
        "$$",
        f"(dR_\\tau/dR_\\mu)_Q = {amp_tau_mu:.15f}.",
        "$$",
        "",
        "## Schur Bounds",
        "",
    ...
    out = Path(__file__).with_name("output_hierarchy_8d_schur_response.md")
