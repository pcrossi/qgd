#!/usr/bin/env python3
"""
GDQ — Chapter 17 / collective surface modes.

Classification:
    reduced test of surface impedance by Schur complement.

The neutron electrical form factor receives probe response:

    I_Sigma(q) = -J_Sigma^dagger K_Sigma^{-1} J_Sigma.

This script projects the reduced impedance onto three minimum collective modes:
normal displacement, shear/magnetization, and non-local torsion. Galster is
used as a compact shape benchmark, not as a fundamental input of GDQ.
"""
...
def main() -> None:
    ...
    if np.any(strengths <= 0.0):
        raise RuntimeError(f"unstable collective mode: {strengths}")
    ...
    metric_rows = []
    for label, curve in [("scalar surface", g_sigma), ("collective modes", g_full)]:
    ...
    lines = [
        "---",
        'title: "Output — collective surface modes"',
        "---",
        "",
        "# Output — collective surface modes",
        "",
        "## Surface Schur complement",
        "",
        "$$",
        "\\mathcal I_\\Sigma(q)",
        "=",
        ...
        "",
        "## Metrics against Galster",
        "",
        "| curve | interval $q$ | RMS | relative RMS |",
        "|---|---:|---:|---:|",
    ]
    for label, lo, hi, abs_rms, rel_rms in metric_rows:
        lines.append(f"| {label} | `{lo:.2f}`–`{hi:.1f}` | `{abs_rms:.6e}` | `{100.0*rel_rms:.3f}%` |")

    lines.extend(["", "## Sample", "", "| $q$ fm$^{-1}$ | GDQ refined | Galster | $\\mathcal I_\\Sigma$ |", "|---:|---:|---:|---:|"])
    ...
    lines.extend(
        [
            ...
            "## Verdict",
            "",
            "The collective modes preserve zero charge and low-energy slope.",
            "The comparison with Galster is a shape benchmark: it informs the size of the",
            "probe's response, but does not alter the official action.",
            "",
        ]
    )

    out = Path(__file__).with_name("output_collective_surface_modes.md")
