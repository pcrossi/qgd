#!/usr/bin/env python3
"""GDQ — Chapter 17 / Angular overlap of four modes."""
...
def main() -> None:
    ...
    lines = [
        "# Output — angular overlap of four beta modes",
        "",
        "Classification: direct evaluation of SU(2) algebra of the four modes.",
        "",
        "```text",
        "S = (p† n)(e† nu)",
        ...
        "```",
        "",
        "## Gram with initial spin average",
        "",
        "```text",
        np.array2string(gram.real, precision=12),
        "```",
        "",
        f"- residue against diag(2,6): `{residual_gram:.3e}`",
        f"- Fierz residue: `{fierz_residual:.3e}`",
        "",
        "Conclusion: the unpolarized sum is `2|C_S|^2 + 6|C_T|^2`; the coefficients must still come from the projected action.",
        "",
    ]
    out = Path(__file__).with_name("output_verify_four_mode_overlap_beta.md")
