#!/usr/bin/env python3
"""
Chapter 4 — absence of Landau pole in the effective U(1) sector.

Classification:
    Direct evaluation and consistency test.

This script evaluates the scalar polarization

    Pi_eta(r)=2 alpha/pi int_0^1 dx x(1-x)
      [E1(eta)-E1(eta*(1+x(1-x)r))]

with r=q_E^2/m^2 and eta=tau*m^2=m^2/Lambda_EM^2.
...
4. condition without pole Pi(infty)<1;
5. approximation of the QED limit when eta is very small.

There is no adjustment to experimental data and eta is only a test scenario.
"""
...
def e1(z: float) -> float:
    """Exponential integral E1(z) for z>0, without external dependencies."""
    if z <= 0:
...
def pi_eta(r: float, eta: float, alpha0: float = ALPHA0, n: int = 800) -> float:
    if r < 0.0 or eta <= 0.0:
        raise ValueError("requires r>=0 and eta>0")
    e0 = e1(eta)
...
def main() -> None:
    ...
    # Tensor test: Pi_mn=(q_m q_n-q^2 delta_mn)Pi. The contraction must vanish.
    ...
    lines += [
        ...
        f"- bounded by asymptotic value: `{bounded}`;",
        f"- no-pole condition in scenario: `{no_pole}`;",
        f"- Ward tensorial residue: `{ward_abs:.3e}`.",
        "",
    ...
    lines += [
        "",
        f"Error between last two orders: `{conv_error:.3e}`.",
        "",
        "## Low energy limit",
        "",
    ...
    out = Path(__file__).with_name("output_verify_absence_landau_pole_u1.md")
