...
"""
Goal:
    Self-contained record of the `reduced klein nishina` verification associated with chapter `24_nuclear_phenomenology`.
Reduced Klein--Nishina verification.

Scientific classification:
    asymptotic reduction consistency test.

The script calculates the differential cross section in units of r_e^2 and verifies the
Thomson limit when x=E/(m_e c^2) tends to zero. The r_e^2 normalization is used as
the angular comparison unit; its complete 8D derivation is outside this
reduced verifier.
"""
...
OUT = Path(__file__).resolve().parent / "output_reduced_klein_nishina.md"
...
def main() -> None:
    ...
    lines: list[str] = []
    lines.append("# Output — reduced Klein--Nishina\n\n")
    ...
    for x in xs:
        ...
        lines.append("| theta(deg) | E'/E | KN/r_e^2 | Thomson/r_e^2 | rel. difference |\n")
        lines.append("|---:|---:|---:|---:|---:|\n")
    ...
    lines.append("## Thomson limit at theta = 90 degrees\n\n")
    lines.append("| x | rel. difference |\n")
    lines.append("|---:|---:|\n")
    theta = pi / 2.0
