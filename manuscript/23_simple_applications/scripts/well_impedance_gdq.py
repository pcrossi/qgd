#!/usr/bin/env python3
"""Chapter 23 — well with reduced GDQ impedance.

Objective:
    Compare the spectrum of a well with finite physical walls against:
    (1) the Robin/DtN condition derived by Schur;
    (2) the direct diagonalization of the finite barrier;
    (3) the ideal infinite well.

Classification:
    Consistency and convergence test. No experimental data is used.

Units:
    L = 1 and hbar^2/(2mL^2)=1.
"""
...
OUT = Path(__file__).with_name("output_well_impedance_gdq.md")
...
def coth(x: float) -> float:
    """Stable hyperbolic cotangent for large x."""
...
def impedance(e: float, v0: float, thickness: float) -> float:
    """Dirichlet--Neumann map of finite homogeneous wall."""
...
def spectral_function(e: float, v0: float, thickness: float) -> float:
    """Spectral equation for well of length L=1 with symmetric walls."""
...
def robin_spectrum(v0: float, thickness: float, modes: int) -> np.ndarray:
    """Locates roots of the Robin/DtN problem."""
    ...
    raise RuntimeError("Insufficient roots below V0.")


def direct_spectrum(v0: float, thickness: float, points: int, modes: int) -> Spectrum:
    """Direct diagonalization of the finite barrier in [-d,1+d]."""
...
def main() -> None:
    ...
    lines += [
        "",
        f"- maximum error in the finest mesh: `{np.max(err_direct):.3e}`.",
