#!/usr/bin/env python3
"""
GDQ — Chapter 11 / Hopf--Bismut Triplet

Self-contained verification of the facts used in the text:

1. in the standard complex orientation of C^2 ~= R^4, the hyperkähler triplet
   associated with the Hopf map is self-dual;
2. after normalization by sqrt(2), its Gram matrix is the identity;
3. the apparatus selects a direction within this triplet, not the triplet.

The script is elementary symbolic-numerical. It does not fit parameters and does not
...
def hodge_matrix() -> np.ndarray:
    """
    Hodge in R^4 oriented by e1^e2^e3^e4 > 0:
...
def main() -> None:
    ...
    lines += [
        "",
        "## Gram matrix of the normalized basis Sigma_i^+ = Omega_i/sqrt(2)",
        "",
        "```text",
        np.array2string(gram, precision=12, suppress_small=False),
        "```",
        "",
        "Conclusion: the Hopf triplet is self-dual in the standard complex orientation and the normalized basis is orthonormal.",
        "",
    ]

    out = Path(__file__).with_name("output_verify_sg_hopf_bismut_triplet.md")
