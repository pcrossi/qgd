#!/usr/bin/env python3
"""
Chapter 11 — Spectrum of the two Robin channels of Stern–Gerlach.
...
In the exterior, natural Neumann is used. The linear finite element discretization
generates the generalized symmetric problem:
    K v = lambda M v.

The dimensionless spectral sums are:
    Gamma_red = sum |j_nu|^2 / lambda_nu,
    kappa_red = sum |j_nu|^2 / lambda_nu^2.

They test structure and convergence; they are not physical rates without the calibration
of the GDQ radial background.
"""
...
def assemble_fem(
    ...
    # Left Robin with outer normal: -psi'(r_c)+R psi(r_c)=0.
    K[0, 0] += robin_left
...
def solve_channel(
    ...
    # eigsh normalizes in the M metric; we reinforce it to reduce accumulated error.
...
def run(args: argparse.Namespace) -> str:
    ...
    lines = [
        ...
        "$$",
        "",
        f"- domain: [{args.r_c}, {args.r_max}]",
        f"- R0: {args.robin0}",
        f"- separation rB: {args.robin_split}",
        f"- mass2: {args.mass2}",
        f"- well: {args.well}",
        f"- well width: {args.well_width}",
        f"- modes in sums: {args.modes}",
        "",
        "## Convergence",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- fundamental mode separation lambda1+ - lambda1-: {splitting:.9e}",
            f"- both gaps positive: {bool(plus.eigenvalues[0] > 0 and minus.eigenvalues[0] > 0)}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).with_name("output_robin_channels_sg.md"),
    )
