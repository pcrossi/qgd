#!/usr/bin/env python3
"""
Chapter 11 — Spatial propagation of a Stern–Gerlach beam.

The channel of each trajectory is generated by the conditioned SDE:
    dp = 4 sqrt(Gamma) p(1-p) dW.

After the capture of channel s = ±1, the particle undergoes constant transverse
acceleration s*a_sg inside the magnet and drifts freely to the screen.

The script tests:
  - weights of the two channels;
  - centers of the spots;
  - separation between the spots;
  - internal dispersion;
  - agreement with the analytical kinematics.
"""
...
from simulate_sg_capture import born_probability, logit, sigmoid
...
def sample_channels(
    ...
    # Unconverged are rare in the standard regime; we preserve the zero status.
...
def simulate_beam(args: argparse.Namespace) -> str:
    ...
    lines = [
        "# Spatial propagation Stern–Gerlach — Chapter 11",
        "",
        "## Parameters",
        "",
        f"- particles: {args.particles}",
        f"- theta: {args.theta} degrees",
        f"- Born p+: {p0:.9f}",
        f"- first passage p+: {threshold_target:.9f}",
        f"- time in magnet: {time_magnet:.6f}",
        f"- drift time: {time_drift:.6f}",
        f"- analytical displacement per channel: {displacement:.9f}",
        "",
        "## Result",
        "",
        f"- resolved: {channels.size}",
        f"- channel +: {plus.size}",
        f"- channel -: {minus.size}",
        f"- frequency +: {p_mc:.9f}",
        f"- center +: {center_plus:.9f}",
        f"- center -: {center_minus:.9f}",
        f"- numerical separation: {separation:.9f}",
        f"- analytical separation: {analytic_separation:.9f}",
        f"- relative separation error: {relative_error:.3e}",
        f"- sigma +: {sigma_plus:.9f}",
        f"- sigma -: {sigma_minus:.9f}",
        f"- expected kinematic sigma: {expected_sigma:.9f}",
        f"- average capture time: {float(np.mean(capture_times)):.9f}",
        "",
        "## Verdict",
        "",
        "The weights come from the conditioned dynamics; the spot positions come from the",
        "opposing force in the two channels. The acceleration used is dimensionless and must",
        "be replaced by mu*grad(|B|)/m in a physical simulation.",
        "",
    ]
    return "\n".join(lines)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).with_name("output_beam_sg.md"),
    )
