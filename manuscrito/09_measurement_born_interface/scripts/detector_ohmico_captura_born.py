#!/usr/bin/env python3
"""Detector ôhmico reduzido e captura Born.

Classificação:
    teste de consistência com parâmetros adimensionais.

Este script verifica a nota:

    notes/detector_ohmico_captura_born.md

Ele não avalia um material físico. Os parâmetros são fixos e diagnósticos. O
teste cobre:

    1. convergência do DtN retardado de canal aberto;
    2. taxa informacional Gamma = g_X^2/(8 gamma_A kBT);
    3. martingal E[p_t] = p0;
    4. erro finito de classificação;
    5. relaxação do ponteiro para os registros condicionados.
"""
...
OUT = Path(__file__).resolve().parent / "saida_detector_ohmico_captura_born.md"
...
def dtn_errors(p: Params) -> list[tuple[float, float]]:
    """Confere Lambda_ret = -i omega zeta_A/c_A por derivada unilateral."""
...
def main() -> None:
    ...
    lines = ["# Saída — detector ôhmico e captura Born\n\n"]
    lines.append("Classificação: teste de consistência com parâmetros adimensionais.\n\n")
    lines.append("## Parâmetros diagnósticos\n\n")
    ...
    for name, value in (
        ...
        ("p0", p.p0),
        ("trajetorias", float(p.n_traj)),
        ("tempo final", p.total_time),
    ):
        lines.append(f"| {name} | {value:.12f} |\n")

    lines.append("\n## Convergência do DtN retardado\n\n")
    lines.append("| h | erro relativo |\n")
    lines.append("|---:|---:|\n")
    for h, err in rows:
    ...
    lines.append("|---|---:|\n")
    lines.append(f"| erro martingal | {martingale_error:.12e} |\n")
    lines.append(f"| erro padrão binomial | {sampling_se:.12e} |\n")
    lines.append(f"| erro martingal em desvios padrão | {martingale_error / sampling_se:.12f} |\n")
    lines.append(f"| diferença erro MC vs analítico | {error_agreement:.12e} |\n")
    lines.append("\n## Veredito\n\n")
    lines.append(
        "O teste confirma o DtN ôhmico, a propriedade de martingal, a captura "
        "assintótica condicionada e a relaxação do ponteiro no modelo reduzido. "
        "Não calcula parâmetros de material real.\n"
    )
