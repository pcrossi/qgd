...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=Path(__file__).with_name(
            "saida_verificar_estabilidade_raio_cilindrico_sg.md"
        ),
    )
    ...
    lines = [
        "# Saída — estabilidade homogênea do raio cilíndrico",
        "",
        "| quantidade | valor |",
        "|---|---:|",
        f"| $a_*$ | {radius:.12e} |",
        ...
        f"| $W''$ analítica | {hessian:.12e} |",
        f"| $W''$ diferença finita | {finite_hessian:.12e} |",
        "",
        f"- modo homogêneo estável: `{hessian > 0}`",
    ]
    report = "\n".join(lines) + "\n"
