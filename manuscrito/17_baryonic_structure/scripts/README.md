...
| `derivacao_simbolica_massas_barioes.py` | Derivar simbolicamente $M_p/M_e$, $\delta_B$ e $M_n/M_e$. | Derivação simbólica autocontida. |
| `calcular_massas_barioes.py` | Avaliar massas reduzidas de próton/nêutron. | Avaliação direta. |
...
| `calcular_raios_momentos_barioes.py` | Avaliar raio do próton e momentos magnéticos reduzidos. | Avaliação direta. |
...
| `modos_coletivos_superficie.py` | Avaliar a impedância coletiva de superfície por Schur em três modos. | Teste reduzido de resposta de sonda. |
...
| `validar_beta_livre_completo.py` | Calcular endpoint, espaço de fase analítico/Simpson, norma contraída, vida média, meia-vida, espectro contínuo reduzido e comparação. | Avaliação direta, teste de convergência e comparação fenomenológica. |
| `escala_eletronica_beta.py` | Determinar $M_ec^2$ a partir de $Q_\beta$ e $\delta_B$. | Determinação metrológica reduzida. |
| `comparar_tau_neutron.py` | Avaliar $\tau_n$ reduzido e comparar com referência. | Comparação fenomenológica. |
