#!/usr/bin/env python3
"""Capítulo 16 — variações superiores da ação GDQ reduzida.

Objetivo:
    Calcular, sem usar dados experimentais, alguns coeficientes cúbicos e
    quárticos da ação oficial reduzida usada na auditoria Galerkin da Capítulo 16.
...
GALERKIN = BASE / "hessiana_oficial_galerkin_gmenos2.py"
ALPHA = 1.0 / 137.035999177


def load_galerkin_action() -> Callable[[np.ndarray], float]:
    spec = importlib.util.spec_from_file_location("hessiana_oficial_galerkin_gmenos2", GALERKIN)
...
def main() -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "## 6. Consequência para Capítulo 16",
            "",
            ...
            "",
            "Assim, a Capítulo 16 ganha uma conclusão adicional: a ação reduzida permite",
    ...
    out = BASE / "saida_variacoes_superiores_gdq_gmenos2.md"
